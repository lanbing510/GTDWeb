...
(function($) {
	$.fn.glanceyear = function(massive, options) {
		...
		//Weeks
		for (var i=0; i<54; i++) {
			var gElement = createElementSvg('g', {'transform': 'translate('+(16*i)+',0)'} );   
			...
			for (var j=firstDate.getDay(); j<7 ; j++) {
				...
				if ( rectDate.getMonth() != monthCount && i < 52 && j > 3 && daysLeft > 7) {
					//new Month
					var textMonth = createElementSvg('text', {'x': 16*i, 'y':'-6', 'class':'month'} );
